...
impl LayoutBridge {
    ...
    fn convert_coords(
        ...
        #[cfg(target_os = "macos")]
        {
            // macOS typically uses bottom-left origin, BUT winit's content view is flipped (top-left origin).
            // Taffy uses top-left origin.
            // Since the target view is flipped, we can use Taffy's coordinates directly.
    ...
    pub fn native_to_taffy_point(x: f64, y: f64, _parent_height: f32, _scale_factor: f32) -> Vec2 {
        #[cfg(target_os = "macos")]
        {
            // macOS typically uses bottom-left, but our content view is flipped (top-left)
...
mod tests {
    ...
    fn test_coordinate_conversion_macos_bottom_left() {
        ...
        // Expected macOS Y: 550 (flipped view means top-left origin)
        #[cfg(target_os = "macos")]
        {
            let (x, y, w, h) = LayoutBridge::convert_coords(0.0, 550.0, 100.0, 50.0, 600.0, 2.0);
            assert_eq!(x, 0.0, "X should be 0.0");
            assert_eq!(y, 550.0, "Y should be 550.0 (no flip)");
            assert_eq!(w, 100.0, "Width should be 100.0");
            assert_eq!(h, 50.0, "Height should be 50.0");
        }
    }
    ...
    fn test_coordinate_conversion_macos_negative_y_clamped() {
        // Test: Widget that results in negative Y
        // This shouldn't happen with taffy usually, but just in case
        #[cfg(target_os = "macos")]
        {
            let (x, y, w, h) = LayoutBridge::convert_coords(0.0, -10.0, 100.0, 200.0, 600.0, 2.0);
            assert_eq!(x, 0.0, "X should be 0.0");
            assert_eq!(y, -10.0, "Y should be preserved (no clamping by bridge)");
            assert_eq!(w, 100.0, "Width should be 100.0");
    ...
    fn test_coordinate_conversion_macos_zero_size_minimum() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            assert_eq!(y, 200.0, "Y should be 200.0");
    ...
    fn test_coordinate_conversion_macos_negative_x_clamped() {
        // Test: Widget with negative X
        #[cfg(target_os = "macos")]
        {
            let (x, y, w, h) = LayoutBridge::convert_coords(-50.0, 100.0, 100.0, 50.0, 600.0, 2.0);
            assert_eq!(x, -50.0, "X should be preserved");
            assert_eq!(y, 100.0, "Y should be 100.0");
            assert_eq!(w, 100.0, "Width should be 100.0");
            assert_eq!(h, 50.0, "Height should be 50.0");
        }
    }
    ...
    fn test_bounds_to_frame_conversion() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            assert_eq!(y, 100.0, "Y should be 100.0 (no flip)");
            assert_eq!(w, 200.0, "Width should be 200.0");
    ...
    fn test_coordinate_conversion_different_window_heights() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            assert_eq!(
                y1, 100.0,
                "Y should be 100.0 regardless of window height (no flip)"
            );
    ...
    fn test_native_to_taffy_point_conversion() {
        ...
        #[cfg(target_os = "macos")]
        {
            // Native point at bottom-left (x=50, y=600 in macOS flipped coordinates)
            // Should convert to taffy point at bottom (y=600 in taffy)
            let point = LayoutBridge::native_to_taffy_point(50.0, 600.0, 600.0, 2.0);
            assert_eq!(
                point.x, 50.0,
                "X should be 50.0"
            );
            assert_eq!(point.y, 600.0, "Y should be 600.0 (flipped view: direct mapping)");

            // Native point at top-left (x=50, y=50 in macOS flipped coordinates)
            // In taffy, this should be near the top: y=50
            let point2 = LayoutBridge::native_to_taffy_point(50.0, 50.0, 600.0, 2.0);
            assert_eq!(point2.x, 50.0, "X should be 50.0");
            assert_eq!(point2.y, 50.0, "Y should be 50.0");
        }
    }
    ...
    fn test_multiple_widgets_same_container() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            // Y positions should be in normal order (macOS flipped content view)
            // Widget 1 (top in taffy) should have lowest Y in macOS (top-left origin)
            // Widget 3 (bottom in taffy) should have highest Y in macOS
            assert!(
                y1 < y2,
                "Widget 1 should be above widget 2 (lower Y) in macOS coordinates"
            );
            assert!(
                y2 < y3,
                "Widget 2 should be above widget 3 (lower Y) in macOS coordinates"
            );

            // Verify the spacing is correct
            assert_eq!(y2 - y1, 30.0, "Spacing between widget 1 and 2 should be 30");
            assert_eq!(y3 - y2, 30.0, "Spacing between widget 2 and 3 should be 30");
        }
    }
    ...
    fn test_very_large_coordinates() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            // Y should be preserved (no flip means no clamping to 0 needed for positive values)
            assert_eq!(x, 10000.0, "X should preserve large value");
            assert_eq!(y, 50000.0, "Y should be preserved");
    ...
    fn test_fractional_coordinates() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            // Y: 200.25 (no flip)
            assert_eq!(y, 200.25, "Y should preserve fractional value");
